...
it('adds new memo to state when it receives a valid ADD_MEMO action', () => {
  ...
  const result = memoReducer(defaultState, {type: 'ADD_MEMO', memo: newMemo});
  expect(result).toEqual([newMemo, ...defaultState]);
});
